...
async function imageSize(url){
 ...
 try{
  const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0","Accept":"image/*"},redirect:"follow",signal:controller.signal});
  if(!r.ok)return null;
  const b=new Uint8Array(await r.arrayBuffer());let w=0,h=0;
...
export async function GET(request){
 ...
 const candidates=[],verified=[],checked=new Set(),cfg=PROVIDERS[provider.toLowerCase()];
 ...
 async function tavily(query,source){
  ...
  try{
   ...
   diagnostics.aiStatus="http-"+r.status;if(!r.ok)return;
   const d=await r.json(),fresh=[];diagnostics.imagesReturned+=(d.images||[]).length;
   for(const im of (d.images||[])){
    const url=typeof im==="string"?im:(im.url||im.image_url),desc=typeof im==="string"?"":(im.description||"");
    if(url&&imageMatch(name,url,desc)===1){diagnostics.exactMatches++;if(!candidates.some(x=>x.url===url)){const c={url,source,confidence:99};candidates.push(c);fresh.push(c)}}
   }
   await verify(fresh);
  }catch(e){diagnostics.aiStatus="error-"+(e?.name||"unknown")}finally{clearTimeout(timer)}
 }
 await tavily(name+" "+provider+" casino square game tile","AI web search");
 if(aiKey&&!verified.length)await tavily(name+" "+provider+" 1:1 square casino lobby thumbnail game icon artwork","AI square-tile search");
